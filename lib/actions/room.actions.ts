  ...
  try {
    ...
    if (room) {
      ...
      await liveblocks.triggerInboxNotification({
        ...
        activityData: {
          userType,
          title: `${updatedBy.name} shared a document with you (${userType})`,
...
export const removeCollaborator = async ({
  ...
  try {
    ...
    if (room.metadata.email === email) {
      throw new Error("You cannot remove the owner from the document");
    }
...
export const deleteDocument = async (roomId: string) => {
  try {
    ...
    console.log(`Error happened while delting a room: ${error}`);
  }
};
